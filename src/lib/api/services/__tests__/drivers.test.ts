...
describe('drivers service', () => {
  ...
  it('createMyDriverProfile → POST /drivers with role:driver + snake_case body, mapped via transformDriver', async () => {
    const post = vi
      ...
      .mockReturnValue(ok({ id: 'd1', user_id: 'u1', display_handle: 'A1B2C3D', full_name: 'Ravi Kumar', phone: '+919999999999', kyc_status: 'pending' }) as never);
  ...
  it('createMyDriverProfile omits email when not supplied', async () => {
    const post = vi.spyOn(apiClient, 'post').mockReturnValue(ok({ id: 'd2', user_id: 'u2', display_handle: 'A1B2C3D', }) as never);
  ...
  it('createMyAgentProfile → POST /drivers with role:trip_manager + business_city_id, mapped via transformAgent', async () => {
    const post = vi
      .spyOn(apiClient, 'post')
      .mockReturnValue(ok({ id: 'a1', user_id: 'u3', display_handle: 'A1B2C3D', full_name: 'Agent A', phone: '+91', business_name: 'A Travels', kyc_status: 'pending' }) as never);
  ...
  it('getDriver → GET /drivers/:id', async () => {
    const get = vi.spyOn(apiClient, 'get').mockReturnValue(ok({ id: 'd9', user_id: 'u9', display_handle: 'A1B2C3D', }) as never);
  ...
  it('getMyDriver → GET /drivers/me', async () => {
    const get = vi.spyOn(apiClient, 'get').mockReturnValue(ok({ id: 'd1', user_id: 'u1', display_handle: 'A1B2C3D', full_name: 'Me' }) as never);
  ...
  it('getMyAgent → GET /agents/me', async () => {
    const get = vi.spyOn(apiClient, 'get').mockReturnValue(ok({ id: 'a1', user_id: 'u1', display_handle: 'A1B2C3D', full_name: 'Me Agent', business_name: 'My Travels' }) as never);
  ...
  it('getAgents({ kycStatus }) → GET /agents?kyc_status=', async () => {
    const get = vi.spyOn(apiClient, 'get').mockReturnValue(ok([{ id: 'a1', user_id: 'u1', display_handle: 'A1B2C3D', kyc_status: 'docs_submitted' }]) as never);
  ...
  it('updateDriverKyc → PATCH /drivers/:id/kyc { kyc_status, note? }', async () => {
    const patch = vi.spyOn(apiClient, 'patch').mockReturnValue(ok({ id: 'd1', user_id: 'u1', display_handle: 'A1B2C3D', kyc_status: 'approved' }) as never);
  ...
  it('updateAgentKyc → PATCH /agents/:id/kyc', async () => {
    const patch = vi.spyOn(apiClient, 'patch').mockReturnValue(ok({ id: 'a1', user_id: 'u1', display_handle: 'A1B2C3D', kyc_status: 'approved' }) as never);
  ...
  it('updateDriverLocation → PATCH /drivers/:id/location with snake_case lat/lng + a last-seen timestamp', async () => {
    const patch = vi.spyOn(apiClient, 'patch').mockReturnValue(ok({ id: 'd1', user_id: 'u1', display_handle: 'A1B2C3D', current_lat: 13.05, current_lng: 80.2 }) as never);
  ...
  it('updateDriverLocation includes current_city_id when a cityId is supplied', async () => {
    const patch = vi.spyOn(apiClient, 'patch').mockReturnValue(ok({ id: 'd1', user_id: 'u1', display_handle: 'A1B2C3D', }) as never);
