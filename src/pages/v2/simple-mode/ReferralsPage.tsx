  ...
  return (
    <div className="flex min-h-dvh flex-col bg-page pb-6">
      <header className="flex items-center gap-3 px-5 pt-5 pb-3">
        ...
        <div>
          <div className="text-[22px] font-bold">Friend money</div>
          <div className="text-[14px] text-muted-foreground">Get money when a friend joins</div>
        </div>
      </header>
        ...
        {query.isLoading ? (
          ...
          <>
            <article className="rounded-card border-4 border-warning bg-[var(--skin-simple-wait-bg)] p-6 text-center">
              <div className="text-[16px] font-semibold">You earned so far</div>
              ...
              <div className="mt-2 flex items-center justify-center gap-2 text-[14px] text-muted-foreground">
                <Users className="size-4" /> {s?.counts.totalReferred ?? 0} {(s?.counts.totalReferred ?? 0) === 1 ? 'friend joined' : 'friends joined'}
              </div>
            </article>

            <article className="rounded-card border-2 border-border bg-surface p-5 text-center">
              <div className="text-[14px] text-muted-foreground">Your code</div>
            ...
            >
              <MessageCircle className="size-6" /> Send on WhatsApp
            </button>
            <button
              type="button"
            ...
            >
              <Phone className="size-5" /> Call a friend
            </button>
          </>
        )}
      </main>
