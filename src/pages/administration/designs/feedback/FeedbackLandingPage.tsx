...
/**
 * /administration/designs/feedback — questionnaire hub.
 *
...
export function FeedbackLandingPage() {
  ...
  async function onSubmit() {
    ...
    try {
      ...
      reset();
      navigate('/administration/designs/feedback/results');
    } catch (e) {
  ...
  return (
    <main className="mx-auto max-w-2xl space-y-5 p-6">
      <Link to="/administration/designs" className="-ml-1 inline-flex items-center gap-1 text-sm text-secondary hover:text-foreground">
        ...
        <ul className="divide-y divide-border rounded-card bg-surface shadow-card">
          {PREFERENCE_PAGES.map((p) => {
            ...
            return (
              <li key={p.pageKey}>
                <Link
                  to={`/administration/designs/feedback/page/${p.pageKey}`}
        ...
        <ul className="grid grid-cols-1 gap-2 sm:grid-cols-2">
          {SKIN_VERSIONS.map((v) => {
            ...
            return (
              <li key={v}>
                <Link
                  to={`/administration/designs/feedback/sus/${v}`}
      ...
      <Section title="Cross-page synthesis (1)">
        <Link
          to="/administration/designs/feedback/cross-page"
          className="flex items-center justify-between rounded-card bg-surface px-4 py-3 shadow-card hover:bg-muted"
        >
          <span className="flex items-center gap-2">
      ...
      <p className="text-xs text-secondary">
        <Link to="/administration/designs/feedback/results" className="underline">See aggregated results</Link>
      </p>
    </main>
  );
}
