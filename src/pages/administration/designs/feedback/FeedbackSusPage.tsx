...
/** /administration/designs/feedback/sus/:design — SUS questionnaire for one design. */
  ...
  return (
    <main className="mx-auto max-w-2xl space-y-5 p-6">
      ...
      <nav className="flex items-center justify-between border-t border-border pt-4">
        {prev ? (
          <Button variant="outline" onClick={() => navigate(`/administration/designs/feedback/sus/${prev}`)} className="gap-1">
            <ChevronLeft className="size-4" /> {prev.toUpperCase()}
          </Button>
        ) : <span />}
        {next ? (
          <Button onClick={() => navigate(`/administration/designs/feedback/sus/${next}`)} className="gap-1">
