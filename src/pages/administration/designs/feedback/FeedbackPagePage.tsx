...
/** /administration/designs/feedback/page/:page — preference questions for a single page across all 6 versions. */
  ...
  if (!page) {
    return (
      <main className="mx-auto max-w-2xl p-6">
        <Link to="/administration/designs/feedback" className="-ml-1 inline-flex items-center gap-1 text-sm text-secondary hover:text-foreground">
  ...
  return (
    <main className="mx-auto max-w-2xl space-y-5 p-6">
      ...
      <nav className="flex items-center justify-between border-t border-border pt-4">
        {prev ? (
          <Button variant="outline" onClick={() => navigate(`/administration/designs/feedback/page/${prev.pageKey}`)} className="gap-1">
            <ChevronLeft className="size-4" /> {prev.label}
          </Button>
        ) : <span />}
        {next ? (
          <Button onClick={() => navigate(`/administration/designs/feedback/page/${next.pageKey}`)} className="gap-1">
            {next.label} <ChevronRight className="size-4" />
          </Button>
        ) : (
          <Button onClick={() => navigate('/administration/designs/feedback')} className="gap-1">
