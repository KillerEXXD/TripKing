...
3. 🔶 **Core marketplace** — DB: migrations 002–004 applied (`users` + `handle_new_user` + `is_admin()`; `drivers`/`trip_managers`/`vehicles` via FK lookups; `trips`/`trip_acceptances`/`trip_executions` with the `driver_payout`/`applicant_count` triggers; `vacancies`+`vacancy_destinations`/`alerts`/`reviews`/`notifications`; admin-write RLS; `get_admin_dashboard()` + `get_agent_analytics()`). **Data layer done for all 10 marketplace resources** — types + strict transforms + services + React Query hooks + transform tests. **All 11 edge functions deployed + smoke-verified (+ OpenAPI):** `/admin`, `/auth`, `/trips`, `/notifications`, `/vehicles`, `/drivers`, `/agents`, `/vacancies`, `/alerts`, `/reviews`, `/analytics` — incl. admin KYC review (`PATCH /drivers|agents/:id/kyc`, CSV `?kyc_status=` queue), reviews moderation (`?flagged=true`, `POST /reviews/:id/moderate`), vehicle-eligibility filters (`?eligibility=`, `?needs_attention=true`), `/analytics/admin` + `/analytics/agent`. The `/admin/*` mutations require a `role=admin` Bearer (the `X-Admin-Key` stopgap is removed; audit rows record `actor_user_id`). `/auth` is a DEV/PLACEHOLDER: `request-otp` is a no-throw placeholder returning `dev_otp:'12345'`; `verify-otp` accepts `12345` or any 4–6 digit. Every `/trips` row joins the assigned driver with live position; in-progress trips get `distance_to_destination_km` (trip-manager live map = `GET /trips?status=in_progress&posted_by_user_id=`). **Two conflict-free parallel lanes, separate worktrees (see `docs/CONTINUE_HERE.md` → "Running two sessions in parallel"):** *backend lane* (`docs/CONTINUE_HERE_BACKEND.md`, worktree `C:\Apps\TripKing-backend` branch `backend-lane`) — through Phase 5 + Phase-6 step 1 + the frontend-ask endpoints + live tracking + the `/auth` OTP placeholder; **next = the remaining Phase-6 items (real SMS + `auth_otps` — needs a provider decision; rate limiting; `api_metrics` persistence; k6 load suite) — see `docs/SECURITY_REVIEW.md`**; *frontend lane* (`docs/CONTINUE_HERE_FRONTEND.md`, `C:\Apps\TripKing` branch `main`) — the ~15 Phase-3 screens then the Phase-4 admin-ops UIs + Phase-5 dashboards (all backed) on the existing hooks, matching the `C:\Apps\DriverMahal` prototype.
4. 🔶 **Admin operations** — backend done (KYC review/queue, vehicle-eligibility filters, reviews moderation, translation = `/admin/languages`); UIs TODO (KYC video console, eligibility dashboard, translation manager, moderation queue).
5. 🔶 **Analytics** — backend done (`/analytics/admin`, `/analytics/agent` — server-computed); UIs TODO (agent analytics + admin dashboards).
6. 🔶 **Hardening** — ✅ `/admin` `role=admin` Bearer (X-Admin-Key removed) + `docs/SECURITY_REVIEW.md` (RLS-coverage audit) + the frontend-ask endpoints + trip-manager live tracking + the `/auth` OTP placeholder + `passenger_otp_hash` stripped from all `/trips` responses; TODO: real SMS provider + `auth_otps` for `/auth` (+ gate `role:'admin'` for prod) — needs a provider decision; rate limits; `api_metrics` persistence; k6 load suite; perf pass; a11y audit (frontend).
...
| ~~`ADMIN_API_KEY`~~ (was the `/admin/*` stopgap) | `tk_admin_7cecc2a7400cc67d6553f778432b95449a6f27f56e476547` — **NO LONGER USED.** `/admin/*` mutations now require a `role=admin` Bearer (validated in the function). The env var (in `.env.development`) and the `admin` function secret are dead — safe to delete. To act as an admin in dev: `POST /auth/verify-otp { phone, otp:'12345', role:'admin' }` (any 4–6 digit code works). |
