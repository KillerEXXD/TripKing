...
6. 🔶 **Hardening** — ✅ `/admin` `role=admin` Bearer (X-Admin-Key removed) + `docs/SECURITY_REVIEW.md` (RLS-coverage audit) + the frontend-ask endpoints (`GET /drivers/me`, `GET /agents/me`, public `GET /trips/by-otp/:otp`); TODO: real SMS provider + `auth_otps` for `/auth` + gate `verify-otp role:'admin'` for prod; rate limits; `api_metrics` persistence; load tests; perf pass; a11y audit.
