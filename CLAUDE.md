...
6. 🔶 **Hardening** — ✅ `/admin` `role=admin` Bearer (X-Admin-Key removed) + `docs/SECURITY_REVIEW.md` (RLS-coverage audit); TODO: the frontend-ask endpoints; real SMS provider + `auth_otps` for `/auth` + gate `verify-otp role:'admin'` for prod; rate limits; `api_metrics` persistence; load tests; perf pass; a11y audit.
