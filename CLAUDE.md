...
3. 🔶 **Core marketplace** — DB: migrations 002 & 003 applied (`users` + `handle_new_user` + `is_admin()`; `drivers`/`trip_managers`/`vehicles` via FK lookups; `trips`/`trip_acceptances`/`trip_executions` with the `driver_payout`/`applicant_count` triggers; `vacancies`+`vacancy_destinations`/`alerts`/`reviews`/`notifications`; admin-write RLS on the migration-001 tables). **Data layer done for all 10 resources** — types + strict transforms + services + React Query hooks + transform tests (`auth`, `admin-config`, `trips`, `drivers`/`agents`, `vehicles`, `vacancies`, `alerts`, `reviews`, `notifications`). **All 10 marketplace edge functions + the Phase-4 admin-ops backend deployed + smoke-verified (+ OpenAPI):** `/admin`, `/auth`, `/trips`, `/notifications`, `/vehicles`, `/drivers`, `/agents`, `/vacancies`, `/alerts`, `/reviews` — plus admin KYC review (`PATCH /drivers|agents/:id/kyc`, CSV `?kyc_status=` queue), reviews moderation (`?flagged=true`, `POST /reviews/:id/moderate`), vehicle-eligibility filters (`?eligibility=`, `?needs_attention=true`). **TODO — split into two conflict-free parallel lanes (see `docs/CONTINUE_HERE.md` → "Running two sessions in parallel"):** *backend lane* (`docs/CONTINUE_HERE_BACKEND.md`) — Phase-5 backend (server-computed analytics endpoints), then Phase-6 hardening — same recipe; *frontend lane* (`docs/CONTINUE_HERE_FRONTEND.md`) — the ~15 Phase-3 screens (onboarding/KYC, driver/agent home, trip feed/detail, post trip/vacancy, applicant review, posted trips, assigned trip + OTP, profiles, alerts, passenger portal) then the Phase-4 admin-ops UIs (KYC video console, eligibility dashboard, translation manager, moderation queue — all backed) on the existing hooks, matching the `C:\Apps\DriverMahal` prototype.
