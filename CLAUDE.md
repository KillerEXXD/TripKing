...
- **MCP servers & local config (open the folder → it all loads).** `.mcp.json` (committed) registers 10 MCP servers — `github`, `vercel`, `supabase`, `git`, `sequential-thinking`, `puppeteer`, `stitch`, `nanobanana`, `21st-dev-magic`, `context7` — with `${ENV}` placeholders for the secret-bearing ones. **When unsure of an external library's current API (TanStack Query / shadcn / Supabase / etc.), call `context7` MCP before guessing.** `.claude/settings.json` (committed) sets `enableAllProjectMcpServers: true` (so they load without per-server "approve?" prompts) plus `additionalDirectories` (hudr-pwa, DriverMahal — the reference repos). The credential **values** live in `.claude/settings.local.json` (gitignored), in an `env` block: `GITHUB_PAT`, `VERCEL_MCP_TOKEN`, `SUPABASE_SERVICE_ROLE_KEY` (= the same value as in `.env.development`), `GOOGLE_AI_API_KEY`, `TWENTY_FIRST_API_KEY` — get the values from the team password manager / TournamentPro's `.claude/settings.local.json`. (One-time per machine: trust the workspace; the `npx`-based servers download their package on first connect. MCP config is read at session start — restart Claude Code to pick up changes.)
...
- **Worktrees — when to use them.** Default: small fixes go straight to `main` (Husky pre-push gates each commit). **Spin up a `feat/<thing>` worktree** (`git worktree add -b feat/<thing> ../TripKing-<thing> origin/main`) when **any** of these is true: the change touches more than ~3 files; it spans more than one commit (multi-phase feature); a parallel session is likely to push to `main` while you work (we have lost commits to an automatic rebase mid-flight — recoverable via `git reflog` + `git cherry-pick`, but avoidable); the work is risky enough that an isolated branch + PR review pays for itself. After merge, delete (SAFELY — see the next bullet): `git worktree remove ../TripKing-<thing> && git branch -d feat/<thing>`.
- **NORM: push from an isolated worktree, and clean it up safely.** The shared `C:\Apps\TripKing` checkout is worked by many concurrent sessions that `git checkout` other branches and rewrite files **mid-run**, so the Husky gate (tsc + vitest coverage + build) intermittently fails on things that aren't your code (a test file "Cannot find module …" because it vanished when another session switched branches; `waitFor` timeouts from CPU contention) and HEAD moves under you. So for **any push that must clear the gate**: (1) commit on your `fix/<thing>`/`feat/<thing>` branch; (2) `git worktree add C:/Apps/TripKing-<thing> <branch>`; (3) the fresh worktree has no `node_modules` — junction it: `New-Item -ItemType Junction -Path 'C:\Apps\TripKing-<thing>\node_modules' -Target 'C:\Apps\TripKing\node_modules'`; (4) `cd C:/Apps/TripKing-<thing> && git push -u origin <branch>` so the gate runs against stable files. **CLEANUP TRAP (we wiped the shared `node_modules` once):** a `node_modules` *junction* must be unlinked **before** removing the worktree — `Remove-Item -Force` / `git worktree remove --force` follow the link and delete the shared target's contents (it errors partway with a NullReferenceException, leaving a corrupt tree that breaks every other session). Unlink the reparse-point only with `powershell -Command "[System.IO.Directory]::Delete('C:\Apps\TripKing-<thing>\node_modules', $false)"` (NOT `Remove-Item`; and `cmd /c rmdir` launched from the Bash tool tends to go interactive — don't rely on it), then `rm -rf C:/Apps/TripKing-<thing>` + `git worktree prune`. If you do clobber it, `npm ci` restores it.
- **Branch sanity before every commit.** Always check `git branch --show-current` (or read the `🌿 Committing to: <branch>` line the pre-commit hook prints) BEFORE running `git commit`. A parallel session sharing the same worktree can `git checkout` a different branch out from under you — we landed two commits on the wrong branch in one session before adding the hook. If the wrong branch is shown, abort: `git reset HEAD~ && git checkout main` (or your branch). Same line is also echoed by pre-push as `🌿 Pushing branch:` — a second chance to catch it.
