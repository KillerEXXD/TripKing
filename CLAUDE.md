...
3. 🔶 **Core marketplace** — DB: migrations 002 & 003 applied (`users` + `handle_new_user` + `is_admin()`; `drivers`/`trip_managers`/`vehicles` via FK lookups; `trips`/`trip_acceptances`/`trip_executions` with the `driver_payout`/`applicant_count` triggers; `vacancies`+`vacancy_destinations`/`alerts`/`reviews`/`notifications`; admin-write RLS on the migration-001 tables). **Data layer done for all 10 resources** — types + strict transforms + services + React Query hooks + transform tests (`auth`, `admin-config`, `trips`, `drivers`/`agents`, `vehicles`, `vacancies`, `alerts`, `reviews`, `notifications`). **TODO:** the `/auth` edge function (real phone-OTP `request-otp`/`verify-otp`/`/me`/`refresh` — even a dev-OTP mode that mints a Supabase session), the `/trips/*` + `/drivers/*` + `/vacancies/*` + `/alerts/*` + `/reviews/*` + `/vehicles/*` + `/notifications/*` edge functions (+ OpenAPI + smoke tests), and the ~15 screens (driver/agent home, trip feed/detail, post trip/vacancy, applicant review, posted trips, assigned trip + OTP, profiles, alerts, onboarding/KYC) matching the `C:\Apps\DriverMahal` prototype.
