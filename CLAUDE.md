...
6. 🔶 **Hardening** — ✅ `/admin` `role=admin` Bearer (X-Admin-Key removed) + `docs/SECURITY_REVIEW.md` (RLS-coverage audit) + the frontend-ask endpoints + trip-manager live tracking + the `/auth` OTP placeholder + `passenger_otp_hash` stripped from all `/trips` responses + rate limiting (infra + `verify-otp`) + `api_metrics` persistence (migration 005); TODO: real SMS provider + `auth_otps` for `/auth` (+ gate `role:'admin'` for prod) — needs a provider decision; extend rate limiting beyond `verify-otp`; the `GET /trips` public-PII follow-up; k6 load suite; perf pass; a11y audit (frontend).
