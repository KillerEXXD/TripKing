...
6. ⬜ **Hardening** — replace the `X-Admin-Key` stopgap with a `role=admin` JWT check (the Phase-4/5 routes already use that pattern); real SMS provider + `auth_otps` table for `/auth`; rate limits; RLS-coverage review; load tests; perf pass; a11y audit.
