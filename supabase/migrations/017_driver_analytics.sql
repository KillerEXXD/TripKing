-- TripKing — migration 017: a per-driver analytics function (mirrors get_agent_analytics).
-- Powers GET /analytics/driver (the driver-side "earnings & history" view): resolves p_user_id →
-- drivers.id, then aggregates over the trips assigned to that driver + their trip_acceptances +
-- the reviews they've received. SECURITY DEFINER + search_path=public per the migration-001/002
-- convention. No row-fetch caps (the function aggregates in SQL). Empty driver ⇒ all-zero blob.
...
returns jsonb language sql stable security definer set search_path = public as $$
  ...
  select jsonb_build_object(
    ...
    'earnings_pending',  (select coalesce(sum(driver_payout), 0) from public.trips where assigned_driver_id in (select id from d) and status in ('assigned', 'in_progress')),
