...
| **Rate limiting** on `/auth` and the write endpoints | Abuse / SMS-cost / brute-force protection. GoTrue rate-limits its own ops (`auth.rate_limit` in `config.toml`), but the synthetic-email `admin.createUser`/`signInWithPassword` path partially bypasses that. (Deliberately *not* added to `request-otp` yet — per the "keep it a benign placeholder" decision.) | A `rate_limits(bucket text pk, window_start timestamptz, count int)` table + a `check_rate_limit(p_bucket, p_max, p_window_sec) returns boolean` SQL fn (atomic upsert+increment) + a `_shared/rateLimit.ts` helper. Apply to `verify-otp` (per-phone), then `request-otp` once real SMS lands, then the write endpoints. |
| **`api_metrics` persistence** | `withTiming` currently only logs; `_shared/timing.ts` has a "TODO once that table exists" note. | Migration for an `api_metrics(endpoint, method, status, duration_ms, created_at)` table + a fire-and-forget insert from `withTiming` (the TournamentPro pattern). Note: `_shared/timing.ts` is bundled by every function, so this requires re-deploying all 11. Observability, not strictly security. |
