...
| ✅ **Rate limiting — infrastructure + `verify-otp`** (migration 005) | Brute-force / abuse protection. | Done: `rate_limits(bucket text pk, window_start, count)` table + `check_rate_limit(p_bucket, p_max, p_window_sec) → boolean` SQL fn (atomic upsert+increment, fixed window) + `supabase/functions/_shared/rateLimit.ts` (`rateLimitOk()`, fails *open*). Applied to `POST /auth/verify-otp` — 20 attempts per phone per 10 min, then 429 (`test-auth.cjs` covers it). **Still TODO:** apply to `request-otp` once real SMS lands (it stays a benign placeholder for now), and to the write endpoints. (Cleanup of stale `rate_limits` buckets — a periodic `delete from rate_limits where updated_at < now() - '1 day'` — is a nice-to-have.) |
| ✅ **`api_metrics` persistence** (migration 005) | Observability. | Done: `api_metrics(id, endpoint, method, status, duration_ms, created_at)` table + a fire-and-forget insert from `withTiming()` (memoized service-role client per isolate; never awaited, never affects the response; ~90% persist rate on Deno Deploy — fine for metrics). All 11 functions re-deployed. (No read endpoint yet — an admin dashboard / RPC over `api_metrics` is a future nicety.) |
