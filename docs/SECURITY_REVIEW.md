...
| **Strip `passenger_otp_hash` from the rest of the `/trips` `*` selects** | `GET /trips/:id` and `GET /trips/by-otp/:otp` now strip it; the list (`GET /trips`) and the `fullTrip` helper (assign/start/complete/cancel responses) still return `*` which includes the hash. It's a SHA-256 of a 6-digit OTP (brute-forceable) — anyone who can read a trip could recover the OTP and then `GET /trips/by-otp/:otp`. | Small: a `redactTrip(row)` helper applied in those spots, or switch `TRIP_SELECT` to an explicit column list. |
