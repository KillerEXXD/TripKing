...
- **Edge functions deployed & smoke-verified:** `/admin` (master-data CRUD; mutations require a `role=admin` Bearer — `X-Admin-Key` stopgap removed; audit rows record `actor_user_id` — 11/11), `/auth` (phone-OTP, dev-OTP `123456`, real Supabase sessions via synthetic-email users — 7/7), `/trips` (full lifecycle: post / browse with joins / apply / withdraw / reject / assign+OTP-gen / start+OTP-verify / complete / cancel · + `GET /trips/by-otp/:otp` passenger portal — `passenger_otp_hash` stripped from `GET /:id` & `by-otp` — 12/12), `/notifications` (list / mark-read / mark-all — 6/6), `/vehicles` (CRUD + derived `eligibility_status` + eligibility-dashboard filters — 22/22), `/drivers` + `/agents` (profiles + "create my profile" + `GET /me` + location; idempotent profile-create syncs `users.role`; admin KYC review — 34/34), `/vacancies` (browse with filters + joins / post with destinations / cancel — 22/22), `/alerts` (owner-scoped saved-search CRUD — 21/21), `/reviews` (post-trip ratings; published-or-own visibility; trip-completed check; ratee derived; review_received notification; report + admin moderation — 31/31), `/analytics` (server-computed — `/analytics/admin` platform dashboard + `/analytics/agent` per-agent, over the migration-004 SQL functions — 12/12). OpenAPI (`public/docs/openapi.{yaml,json}`) covers all 11. **All 11 edge functions + the Phase-4 admin-ops backend + the Phase-5 analytics backend are live** (migrations 001–004 applied) — the backend lane is through Phase 5; Phase-6 hardening is what's left.
...
## NEXT STEP → **Phase-6 backend, continued** (✅ all 11 edge functions + Phase-4 admin-ops + Phase-5 analytics + the `/admin` `role=admin` Bearer migration + `docs/SECURITY_REVIEW.md` + the "⬆ Frontend ask" endpoints shipped)

Backend lane: through Phase 5 + Phase-6 step 1 + the frontend-ask endpoints (`GET /drivers/me`, `GET /agents/me`, public `GET /trips/by-otp/:otp`) all done. **Remaining Phase-6 hardening:** real SMS + `auth_otps` for `/auth` (needs a provider decision); gate `verify-otp role:'admin'` for prod; rate limiting; `api_metrics` persistence; strip `passenger_otp_hash` from the remaining `/trips` `*` selects — see `docs/SECURITY_REVIEW.md`. Frontend lane: the Phase-3 **screens** (see `docs/CONTINUE_HERE_FRONTEND.md`; driver/agent home + profile + passenger portal are now unblocked by `GET /drivers/me`, `GET /agents/me`, `GET /trips/by-otp/:otp`) → then **Phase 4–6** UIs (the Phase-4 admin-ops UIs and the Phase-5 analytics dashboards can be built now — their backends are live; the `/administration/config` UI's writes work too now). *(Running in parallel? See the lane split above.)*
